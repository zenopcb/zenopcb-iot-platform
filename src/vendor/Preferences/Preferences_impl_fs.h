...
bool ZenoPreferences::begin(const char * name, bool readOnly){
...
bool ZenoPreferences::remove(const char * key){
...
size_t ZenoPreferences::putBytes(const char* key, const void* buf, size_t len){
...
bool ZenoPreferences::isKey(const char* key) {
    if(!_started || !key){
        return false;
    }
    String path = _path + key;
...
String ZenoPreferences::getString(const char* key, const String defaultValue){
    if(!_started || !key){
        return defaultValue;
    }
...
size_t ZenoPreferences::getBytesLength(const char* key){
    if(!_started || !key){
        return -1;
    }
...
size_t ZenoPreferences::getBytes(const char* key, void * buf, size_t maxLen){
    if(!key){
        return 0;
    }
    String path = _path + key;
...
size_t ZenoPreferences::freeEntries() {
    return 1000;
}
