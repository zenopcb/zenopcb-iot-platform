...
protected:
                       ...
                       const char *pwd = nullptr)
  {
    ...
    // ── ZenoPCB PATCH: TCP stack configuration (same as SIM7600 driver) ──
    // Without these, modemSend() won't get +CIPSEND: confirmation URC,
    // causing write() to timeout and return 0 → subscribe/publish fail.
    ...
    // NmRetry = 10 (retransmission count)
    // DelayTm = 0 (no delay before outputting received data)
    // Ack = 0 (don't report "Send ok" string)
    // errMode = 0 (numeric error codes)
    // HeaderType = 1 (+RECEIVE,<link num>,<data length>)
    // AsyncMode = 0 (synchronous command executing)
    // TimeoutVal = 75000 (minimum retransmission timeout ms)
...
protected:
                    ...
                    bool ssl = false, int timeout_s = 75)
  {
    ...
    // ── ZenoPCB PATCH: AT+NETOPEN MUST come FIRST ──────────────────────
    // The A7680C requires the TCP/IP service to be open before ANY TCP
    // command works (AT+CTCPKA, AT+CIPOPEN, etc.).
    // gprsConnect() only activates PDP context (AT+CGACT) but does NOT
    // open the TCP/IP service.  We must call AT+NETOPEN here.

    // Step 1: Check if TCP/IP service is already open
    sendAT(GF("+NETOPEN?"));
    int8_t netState = waitResponse(5000L, GF("+NETOPEN: 1"), GF("+NETOPEN: 0"),
                                   GF("ERROR" AT_NL));
    // netState: 1 = already open, 2 = not open, 3 = error/not supported

    if (netState != 1)
    {
      // Network not open — open it now
      sendAT(GF("+NETOPEN"));
      if (waitResponse(5000L) != 1)
      {
        // AT+NETOPEN returned ERROR — might already be open, verify
        sendAT(GF("+NETOPEN?"));
        if (waitResponse(5000L, GF("+NETOPEN: 1")) != 1)
        {
          return false; // genuinely failed to open network
        }
        waitResponse(); // consume trailing OK
      }
      else
      {
        // Got OK, now wait for the async URC "+NETOPEN: 0" (up to 30s on 4G)
        waitResponse(30000L, GF("+NETOPEN: 0"));
        // Verify network is actually open
        sendAT(GF("+NETOPEN?"));
        if (waitResponse(5000L, GF("+NETOPEN: 1")) != 1)
        {
          return false; // network failed to open
        }
        waitResponse(); // consume trailing OK
      }
    }
    ...
    {
      waitResponse(); // consume trailing OK from NETOPEN? when already open
    }
    // ── END NETOPEN PATCH ──────────────────────────────────────────────

    // +CTCPKA:<keepalive>,<keepidle>,<keepcount>,<keepinterval>
    // NOW safe to call — TCP/IP service is open
    ...
    {
      // TCP (non-SSL) — AT+NETOPEN already called at top of modemConnect()
  ...
  int16_t modemSend(const void *buff, size_t len, uint8_t mux)
  {
    ...
    if (hasSSL)
    {
      // SSL path: wait for OK then +CCHSEND confirmation URC
      if (waitResponse() != 1)
      {
        return 0;
      }
    ...
    {
      // Non-SSL (AT+CIPSEND): wait for +CIPSEND: send confirmation
      // Response format: +CIPSEND: <mux>,<requested>,<confirmed>
      if (waitResponse(10000L, GF(AT_NL "+CIPSEND:")) != 1)
      {
        return 0;
      }
      streamSkipUntil(',');            // Skip mux
      streamSkipUntil(',');            // Skip requested bytes
