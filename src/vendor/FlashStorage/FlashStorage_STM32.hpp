...
//
// Original Copyright (c) 2020 Khoi Hoang <khoih58@gmail.com>.
// Licensed under MIT — full text in lib/ZenoPCB/src/vendor/FlashStorage/LICENSE.
//
// Plan 07-03 (2026-06-03): Renamed upstream `class EEPROMClass` to `class ZenoFlashStorage`
// and `static EEPROMClass EEPROM` to `static ZenoFlashStorage ZenoEEPROM` for brand
// consistency and to avoid colliding with the Arduino-provided `EEPROM` symbol that the
// STM32duino Core also exposes. SPDX MIT header preserved; LICENSE unchanged.
...
#include "utility/stm32_eeprom.hpp"
  ...
  class ZenoFlashStorage
  {
    public:

      ZenoFlashStorage() : _initialized(false), _dirtyBuffer(false), _commitASAP(true) , _validEEPROM(true) {}
  ...
  static ZenoFlashStorage ZenoEEPROM;
