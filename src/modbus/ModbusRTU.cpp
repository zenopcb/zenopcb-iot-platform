...
void ModbusRTUTemplate::task()
{
	...
	{
		for (uint8_t i = 0; i < _len; i++)
			_port->read(); // Skip packet if SlaveId doesn't mach
		_len = 0;
		if (isMaster)
			cleanup();
		return;
	...
	{ // Fail to allocate buffer
		for (uint8_t i = 0; i < _len; i++)
			_port->read(); // Skip packet if can't allocate buffer
		_len = 0;
		if (isMaster)
			cleanup();
		return;
	...
	if (isMaster)
	{
		if ((_frame[0] & 0x7F) == _sentFrame[0])
		{ // Check if function code the same as requested
...
bool ModbusRTUTemplate::cleanup()
{
	...
	if (_slaveId && (micros() - _timestamp > _timeoutUs))
	{
		if (_cb)
		{
