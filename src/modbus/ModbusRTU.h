...
protected:
	...
	uint32_t _timeoutUs = MODBUSRTU_TIMEOUT_US; // Runtime-configurable timeout
...
public:
	...
	void setTimeout(uint32_t ms) { _timeoutUs = (uint32_t)ms * 1000UL; }
	// Force-abort pending transaction immediately (for write preemption)
	void abort()
	{
		if (_slaveId)
		{
			...
			_data = nullptr;
			_slaveId = 0;
			// Flush RX buffer to prevent stale response corrupting next transaction
			if (_port)
			{
